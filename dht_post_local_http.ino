...
void startWiFi() {
  ...
  if (connect_as_client) {
    ...
    Serial.print("DNS-0:"); Serial.println(WiFi.dnsIP(0));
    Serial.print("DNS-1: "); Serial.println(WiFi.dnsIP(1));
...
void handleRoot(char* apssid) {                         // When URI / is requested, send a web page with a button to perform the actions
...
bool handleRemoveFile() {                          // If a POST request is made to URI /clear  
  String path;
...
bool handleClear() {                          // If a POST request is made to URI /clear  
  String path;
  path="/temp.csv";
  Serial.println("handleClear: " + path);
  if (SPIFFS.exists(path)) {
...
unsigned long getInterval() { // Check if the time server has responded, if so, get the UNIX time, otherwise, return 0
  return update_frequency;
}

unsigned long getDuration() { // Check if the time server has responded, if so, get the UNIX time, otherwise, return 0
...
void loop() {
  ...
  if (timeUNIX != 0 || ignore_ntp) {
    ...
    if (currentMillis - prevTemp > intervalTemp) {  // Every intervalTemp, request the temperature
